...
key.setGlobalKey(['M-a', 'k'], function (ev) {
    BrowserCloseTabOrWindow();
}, 'Close tab / window');

key.setGlobalKey(['M-a', 'u'], function (ev) {
    undoCloseTab();
}, 'Undo closed tab');

key.setGlobalKey(['M-a', 'a'], function (ev, arg) {
    ext.exec("tanything", arg);
}, 'view all tabs', true);

key.setGlobalKey(['M-a', 'r'], function (ev) {
    BrowserReload();
}, 'Reload the page', true);

key.setGlobalKey(['M-a', 'M-r'], function (ev) {
    BrowserReload();
}, 'Reload the page', true);

key.setGlobalKey(['M-a', 'b'], function (ev) {
    BrowserBack();
}, 'Back');

key.setGlobalKey(['M-a', 'f'], function (ev) {
    BrowserForward();
}, 'Forward');
...
key.setGlobalKey(['M-a', 'M-a', 'C-M-r'], function (ev) {
    command.restartApp();
}, 'Restart Firefox');
...
key.setGlobalKey(['M-a', 'm', 'h'], function (ev) {
    let browser = getBrowser();
...
key.setGlobalKey(['M-a', 'm', '5', 'h'], function (ev) {
    let browser = getBrowser();
    if (browser.mCurrentTab.previousSibling) {
        browser.moveTabTo(browser.mCurrentTab, browser.mCurrentTab._tPos - 5);
    } else {
...
key.setGlobalKey(['M-e', 'b'], function (aEvent, aArg) {
    ext.exec("hok-start-background-mode", aArg);
}, 'HoK - Background hint mode', true);

key.setGlobalKey(['M-e', ';'], function (aEvent, aArg) {
    ext.exec("hok-start-extended-mode", aArg);
}, 'HoK - Extented hint mode', true);

key.setGlobalKey(['M-e', 'e'], function (aEvent, aArg) {
    ext.exec("hok-start-continuous-mode", aArg);
}, 'Start continuous HaH', true);

key.setGlobalKey(['M-e', 'y'], function (aEvent, aArg) {
    ext.exec("hok-yank-foreground-mode", aArg);
}, 'Hok - Foreground yank hint mode', true);

key.setGlobalKey(['M-e', 'M-e', 'b'], function (ev, arg) {
    ext.exec("bmany-list-toolbar-bookmarks", arg, ev);
}, 'bmany - List toolbar bookmarks');

key.setGlobalKey(['M-g'], function (ev, arg) {
...
key.setGlobalKey(['M-a', 'M-k'], function (ev) {
    BrowserCloseTabOrWindow();
}, 'Close tab / window');

key.setGlobalKey(['M-a', 'M-f'], function (ev) {
    BrowserForward();
}, 'Forward');

key.setGlobalKey(['M-e', 'M-f'], function (aEvent, aArg) {
    ext.exec("hok-start-foreground-mode", aArg);
}, 'Hok - Foreground hint mode', true);